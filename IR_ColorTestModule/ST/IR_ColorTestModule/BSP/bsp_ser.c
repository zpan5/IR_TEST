...
static CPU_INT08U	*rx_string;
static CPU_INT32U	rx_length;
static CPU_INT32U rx_counter;
/*
*********************************************************************************************************
...
static  CPU_INT08U  BSP_Ser_RdByteUnlocked_Timeout (CPU_INT16U timeout, CPU_INT08U *err);
...
//    RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART3, ENABLE);      /* Enable UART clock.                                   */
  ...
  status = USART3->SR;
	...
	if ((USART3->CR1 & 0x20) != 0x00){
		if (DEF_BIT_IS_SET(status, DEF_BIT_05)) {
			...
			if (BSP_SerRxData == '\n' & rx_string[rx_counter - 1] == '\r') {
				
				rx_string[rx_counter - 1] = '\0';
				USART_ITConfig(USART3, USART_IT_RXNE, DISABLE);
				BSP_OS_SemPost(&BSP_SerRxWait);                         /* Post to the sempahore                              */
			}else {
				rx_string[rx_counter++] = BSP_SerRxData;
...
CPU_INT08U  BSP_Ser_RdByte_Timeout (CPU_INT08U *err)
{
    CPU_INT08U  rx_byte;
    ...
    rx_byte = BSP_Ser_RdByteUnlocked_Timeout(10, err);
...
/*
*********************************************************************************************************
*                                       BSP_Ser_RdByteUnlocked_Timeout()
*
* Description : Receive a single byte.
*
* Argument(s) : timeout
...
* Return(s)   : The received byte
*
* Caller(s)   : BSP_Ser_RdStrLen()
*
* Note(s)     : none.
*********************************************************************************************************
*/

CPU_INT08U  BSP_Ser_RdByteUnlocked_Timeout (CPU_INT16U timeout, CPU_INT08U *err)
{
    ...
    *err = BSP_OS_SemWait(&BSP_SerRxWait, timeout);             /* Wait until a data is received                      */
    ...
    while (DEF_TRUE)
    {
        ...
        if (rx_data == ASCII_CHAR_BACKSPACE) {                 /* Is backspace character                              */
            if (rx_buf_ix > 0) {
				...
				if (rx_data == ASCII_CHAR_DEL ) {                 /* Is backspace character                              */
            if (rx_buf_ix > 0) {
                BSP_Ser_WrByteUnlocked((CPU_INT08U)ASCII_CHAR_DEL );
                rx_buf_ix--;                                   /* Decrement the index                                 */
                p_str[rx_buf_ix] = 0;
						}
        }
...
*               len         The size of the string that will be read.
*
* Return(s)   : state of the receive data.
*				@ 1 Successful 
*				@ 0 Fail
*
* Caller(s)   : Application
...
CPU_INT08U  BSP_Ser_RdStrLen(CPU_CHAR    *p_str,
                       CPU_INT16U   len)
{
    CPU_CHAR     rx_data;
	  CPU_INT16U   i;
	...
	for(i = 0; i < len; i++) {
        rx_data = BSP_Ser_RdByteUnlocked_Timeout(100, &err);
...
**************************************************************************************************
*						BSP_Ser_4_RxString()
*Description:	The function read a byte from UART4;
*Argument(s):	none
*Return(s):		err information
*Caller(s):		Application.
*Note(s):		(1) This function blocks until a data is received.
*				(2)	It can not be called frome an ISR.
*************************************************************************************************
*/
CPU_INT08U BSP_Ser_RdString(CPU_INT08U *string, CPU_INT08U length)
{
	CPU_INT08U err;
...
//	while (1) {
//		BSP_OS_SemWait(&BSP_SerRxWait, 0);
//		string[i++] = BSP_SerRxData;
//		while(1) {
//			err = BSP_OS_SemWait(&BSP_SerRxWait, 0);
//			string[i++] = BSP_SerRxData;
//			
//			if (err == DEF_OK) {
//				if (i > 1 && string[i-2] == '\r' && string[i-1] == '\n') {
//					string[i-2] = '\0';
//					
//					USART_ITConfig(USART3, USART_IT_RXNE, DISABLE);
//					BSP_OS_SemPost(&BSP_SerLock);
...
void  BSP_Ser_WrStrLen (CPU_CHAR  *p_str, CPU_INT16U len)
{
    CPU_BOOLEAN  err;
		CPU_INT16U i;
