...
GPIO_TypeDef*     GPIO_PORT[LEDn] = {GPIOC, GPIOC};

const CPU_INT16U  GPIO_PIN[LEDn]  = {GPIO_Pin_12, GPIO_Pin_13};

const CPU_INT32U  GPIO_CLK[LEDn]  = {RCC_AHB1Periph_GPIOC, RCC_AHB1Periph_GPIOC};

GPIO_TypeDef*		GPIO_PORT_KEY[KEYn] ;

const CPU_INT16U 	GPIO_PIN_KEY[KEYn];
const CPU_INT32U	GPIO_CLK_KEY[KEYn];
...
void  BSP_Init (void)
{
    ...
    BSP_LED_Init();                                               /* Init LEDs.                             *///
		AO_Init();
		I2C_3_Initialization();
		I2C_2_Initialization();
		I2C_1_Initialization();
		BSP_Ser_1_IRDA_Init(2400);
												
		BSP_Can_Init();
//		BSP_KEY_Init();																								/* Init KEYs.														  */
//		BSP_IO_Init();
//		BSP_SPI_1_Init();
		BSP_Ser_Init (115200);
//	BSP_Ser_4_Init(115200);
...
void  BSP_LED_Toggle (CPU_INT08U  led)
{
    ...
    if ((led <= 0) || ( led > LEDn)) {
			for (i = 0; i < LEDn; i++) {
        pin_state  = GPIO_ReadOutputData(GPIO_PORT[i]);
        pin_state ^= GPIO_PIN[i];
        GPIO_SetBits(GPIO_PORT[i],     pin_state  & GPIO_PIN[i]);
        GPIO_ResetBits(GPIO_PORT[i], (~pin_state) & GPIO_PIN[i]);
        return;
			}
		}
...
*                                           BSP_KEY_Init()
*
* Description : Initialize any or all the KEYs on the board.
*
* Argument(s) : led     The ID of the LED to control:
*
*                       0    inialize ALL  KEYs
*                       1    inialize user KEY1
*                       2    inialize user KEY2
*                       3    inialize user KEY3
*                       4    inialize user KEY4
*
* Return(s)   : none.
*
* Caller(s)   : Application.
...
*********************************************************************************************************
*/
static void  BSP_KEY_Init()
{
    GPIO_InitTypeDef  GPIO_InitStructure;
	 CPU_INT08U i;
	 for(i = 0; i < KEYn; i++) {
		 RCC_AHB1PeriphClockCmd(GPIO_CLK_KEY[i], ENABLE);
		 GPIO_InitStructure.GPIO_Pin = GPIO_PIN_KEY[i];
		 GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_IN;
		 GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_UP;
		 GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		 GPIO_Init(GPIO_PORT_KEY[i], &GPIO_InitStructure);
	 }
}
...
*                                             BSP_READ_KEY()
*
* Description : Read Specifid KEYs input data port.
*
* Argument(s) : led     The ID of the KEY to input:
*
*                       0    return NULL
*                       1    return KEY1
*                       2    return KEY2
...
CPU_INT08U  BSP_READ_KEY (CPU_INT08U  key)
{
    if (key <= 0) {
    ...
    return (GPIO_ReadInputDataBit(GPIO_PORT_KEY[key], GPIO_PIN_KEY[key]));
}
